          ...
          <div className="px-5 pt-5 pb-4">
            ...
            <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-4 flex items-center justify-between">
              ...
              <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center">
                <MdHotel className="text-2xl text-white/80" />
              </div>
            </div>
          </div>
        </div>
          ...
          {activities.length === 0 ? (
            <div className="text-center py-14">
              <div className="w-20 h-20 rounded-full bg-purple-50 flex items-center justify-center mx-auto mb-4">
                <MdHotel className="text-4xl text-purple-400" />
              </div>
              <p className="text-gray-700 font-semibold mb-1">Belum Ada Data Hotel</p>
              <p className="text-sm text-gray-500 mb-6">Catat penginapan yang Anda gunakan selama perjalanan</p>
              <button
            ...
            <>
              {activities.map((activity) => (
                ...
                >
                  <div className="flex">
                    <div className="w-1.5 bg-purple-500 flex-shrink-0" />
                        ...
                        <div className="flex-1 min-w-0 pr-3">
                          ...
                          {activity.checkIn && activity.checkOut && (
                            <p className="text-xs text-gray-500 mb-1 flex items-center gap-1">
                              ...
                              {new Date(activity.checkIn).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
                              {' – '}
                              {new Date(activity.checkOut).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                            </p>
                          )}
                          <p className="text-xs text-gray-600">
                            Durasi: <span className="font-medium">{activity.nights} malam</span>
                          </p>
                        </div>
